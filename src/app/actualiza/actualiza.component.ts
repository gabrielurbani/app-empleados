...
export class ActualizaComponent implements OnInit {
  ...
  go:number;
  ngOnInit(): void {
    this.go= parseInt(this.route.snapshot.queryParams['go']);
    this.i = this.route.snapshot.params['id'];
    let empleado: Empleado = this.empleadosService.encontrarEmpleados(this.i);
  ...
  //   this.empleadosService.actualizarEmpleado(this.i, miEmpleado);
  //   this.router.navigate([""]);
